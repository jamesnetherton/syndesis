...
grafana.dashboard.new(
  'Syndesis DB Monitoring Dashboard',
  refresh='1m',
  time_from='now-1h',
)
.addPanel(
  graphPanel.new(
    ...
    prometheus.target(
      'sum(irate(pg_stat_database_tup_fetched{datname=~"syndesisdb"}[5m]))',
      datasource='prometheus',
    )
  ).addTarget(
    prometheus.target(
      'sum(irate(pg_stat_database_tup_returned{datname=~"syndesisdb"}[5m]))',
      datasource='prometheus',
    )
  ),
  {
